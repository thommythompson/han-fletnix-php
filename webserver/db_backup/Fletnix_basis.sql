...
DECLARE @coverImages TABLE (id int, location varchar(255))
INSERT INTO @coverImages VALUES(1, '/img/cover_dark_knight.jpg'),(2, '/img/cover_goodfellas.jpg'),(3, '/img/cover_inception.jpg'),(4, '/img/cover_matrix.jpg'),(5, '/img/cover_saving_private_ryan.jpg'),(6, '/img/cover_skyfall.jpg')

DECLARE @posterImages TABLE (id int, location varchar(255))
INSERT INTO @posterImages VALUES(1, '/img/poster_f_and_f.webp'),(2, '/img/poster_maverick.jpeg'),(3, '/img/poster_skyfall.jpeg'),(4, '/img/poster_the_godfather.jpeg')

DECLARE @videoUrls TABLE (id int, location varchar(255))
INSERT INTO @videoUrls VALUES(1, '/video/trailer_maverick_720p.mp4'),(2, '/video/trailer_godfather_720p.mp4'),(3, '/video/trailer_matrix_720p.mp4')

DECLARE @Descriptions TABLE (id int, string varchar(1024))
...
(2, 'When a beautiful stranger leads computer hacker Neo to a forbidding underworld, he discovers the shocking truth--the life he knows is the elaborate deception of an evil cyber-intelligence.'),
(3, 'After more than thirty years of service as one of the Navy''s top aviators, Pete Mitchell is where he belongs, pushing the envelope as a courageous test pilot and dodging the advancement in rank that would ground him.')

DECLARE @StoryLines TABLE (id int, string varchar(2048))
INSERT INTO @StoryLines
VALUES
(1, 'The Godfather "Don" Vito Corleone is the head of the Corleone mafia family in New York. He is at the event of his daughter''s wedding. Michael, Vito''s youngest son and a decorated WW II Marine is also present at the wedding. Michael seems to be uninterested in being a part of the family business. Vito is a powerful man, and is kind to all those who give him respect but is ruthless against those who do not. But when a powerful and treacherous rival wants to sell drugs and needs the Don''s influence for the same, Vito refuses to do it. What follows is a clash between Vito''s fading old values and the new ways which may cause Michael to do the thing he was most reluctant in doing and wage a mob war against all the other mafia families which could tear the Corleone family apart.'),
(2, 'Thomas A. Anderson is a man living two lives. By day he is an average computer programmer and by night a hacker known as Neo. Neo has always questioned his reality, but the truth is far beyond his imagination. Neo finds himself targeted by the police when he is contacted by Morpheus, a legendary computer hacker branded a terrorist by the government. As a rebel against the machines, Neo must confront the agents: super-powerful computer programs devoted to stopping Neo and the entire human rebellion.'),
(3, 'Code-named Maverick, Pete Mitchell, an impetuous, daredevil Navy-pilot ace, is accepted into Top Gun, Miramar''s elite Fighter School. But there, the impulsive young pilot will have to compete with the best of the best, including Iceman, a brilliant and highly competitive fellow student. Now, Mitchell must give his all; however, his father''s mysterious and untimely demise still haunts him. Can Maverick prove his worth to Charlie, the flying school''s no-nonsense astrophysics instructor? Will he be able to suppress his wild nature to win the prestigious Top Gun Trophy?')
...
BEGIN
    ...
    DECLARE @coverImage VARCHAR(255);
    SELECT TOP 1 @coverImage = location FROM @coverImages
    WHERE id = (SELECT FLOOR(RAND()*(6-1+1))+1);

    DECLARE @posterImage VARCHAR(255);
    SELECT TOP 1 @posterImage = location FROM @posterImages
    WHERE id = (SELECT FLOOR(RAND()*(4-1+1))+1);

    DECLARE @videoUrl VARCHAR(255);
    SELECT TOP 1 @videoUrl = location FROM @videoUrls
    WHERE id = (SELECT FLOOR(RAND()*(3-1+1))+1);

    DECLARE @description VARCHAR(1024);
    SELECT TOP 1 @description = string FROM @Descriptions
    WHERE id = (SELECT FLOOR(RAND()*(3-1+1))+1);

    DECLARE @storyLine VARCHAR(2048);
    SELECT TOP 1 @storyLine = string FROM @StoryLines
    WHERE id = (SELECT FLOOR(RAND()*(3-1+1))+1);

    UPDATE Movie SET cover_image = @coverImage, poster_image = @posterImage, video_url = @videoUrl, description = @Description, story_line = @storyLine WHERE movie_id = @id;
